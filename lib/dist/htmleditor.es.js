...
function yr() {
  return tn || (tn = 1, process.env.NODE_ENV !== "production" && (function() {
    ...
    function d(f, R, H, L, Z, W) {
      ...
      if (v.call(R, "key")) {
        $ = e(f);
        var q = Object.keys(R).filter(function(X) {
          return X !== "key";
        });
        L = 0 < q.length ? "{key: someKey, " + q.join(": ..., ") + ": ...}" : "{key: someKey}", E[$ + L] || (q = 0 < q.length ? "{" + q.join(": ..., ") + ": ...}" : "{}", console.error(
...
React keys must be passed directly to JSX without using spread:
  ...
  <%s key={someKey} {...props} />`,
    ...
    var x = tt, b = Symbol.for("react.transitional.element"), w = Symbol.for("react.portal"), C = Symbol.for("react.fragment"), j = Symbol.for("react.strict_mode"), D = Symbol.for("react.profiler"), N = Symbol.for("react.consumer"), m = Symbol.for("react.context"), g = Symbol.for("react.forward_ref"), y = Symbol.for("react.suspense"), M = Symbol.for("react.suspense_list"), S = Symbol.for("react.memo"), z = Symbol.for("react.lazy"), G = Symbol.for("react.activity"), P = Symbol.for("react.client.reference"), U = x.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE, v = Object.prototype.hasOwnProperty, B = Array.isArray, A = console.createTask ? console.createTask : function() {
      return null;
    };
    x = {
...
var h = br();
const mt = 100, yt = 50, bt = 200, rn = 5, vt = (e) => e < bt, Tt = (e) => e > yt, vr = 96, qe = (e) => Math.round(e * vr), Je = {
  NORMAL: {
    name: "Normal",
    label: 'Normal (1")',
    top: 1,
  ...
  switch (t.type) {
    ...
    case Y.UPDATE_PAGES: {
      const n = Array.isArray(t.payload) ? { pages: t.payload } : t.payload || {}, a = Cn(n.pages || [], e.pageSize), o = Array.isArray(n.pageBreaks) ? n.pageBreaks : Ge(a);
      return {
        ...e,
        pages: a,
...
function Cr() {
  ...
  return ne.AsyncMode = p, ne.ConcurrentMode = u, ne.ContextConsumer = i, ne.ContextProvider = s, ne.Element = t, ne.ForwardRef = d, ne.Fragment = n, ne.Lazy = b, ne.Memo = x, ne.Portal = r, ne.Profiler = o, ne.StrictMode = a, ne.Suspense = c, ne.isAsyncMode = function(g) {
    return m(g) || N(g) === p;
  }, ne.isConcurrentMode = m, ne.isContextConsumer = function(g) {
    return N(g) === i;
...
function Rr() {
  return sn || (sn = 1, process.env.NODE_ENV !== "production" && (function() {
    ...
    var g = p, y = u, M = i, S = s, z = t, G = d, P = n, U = b, v = x, B = r, A = o, O = a, V = c, oe = !1;
    function te(I) {
      return oe || (oe = !0, console.warn("The ReactIs.isAsyncMode() alias has been deprecated, and will be removed in React 17+. Update your code to use ReactIs.isConcurrentMode() instead. It has the exact same API.")), E(I) || m(I) === p;
    }
    function E(I) {
      return m(I) === u;
    }
  ...
  return dt = function(i, p) {
    ...
    function C(E) {
      ...
      function H(Z, W, $, q, K, X, ce) {
        ...
        return W[$] == null ? Z ? W[$] === null ? new w("The " + K + " `" + X + "` is marked as required " + ("in `" + q + "`, but its value is `null`.")) : new w("The " + K + " `" + X + "` is marked as required in " + ("`" + q + "`, but its value is `undefined`.")) : null : E(W, $, q, K, X);
      }
      var L = H.bind(null, !1);
    ...
    function N(E) {
      function f(R, H, L, Z, W) {
        ...
        if (!Array.isArray($)) {
          var q = O($);
          return new w("Invalid " + Z + " `" + W + "` of type " + ("`" + q + "` supplied to `" + L + "`, expected an array."));
        }
    ...
    function y(E) {
      function f(R, H, L, Z, W) {
        if (!(R[H] instanceof E)) {
          var $ = E.name || l, q = te(R[H]);
          return new w("Invalid " + Z + " `" + W + "` of type " + ("`" + q + "` supplied to `" + L + "`, expected ") + ("instance of `" + $ + "`."));
        }
        return null;
      }
      return C(f);
    ...
    function S(E) {
      function f(R, H, L, Z, W) {
        ...
        var $ = R[H], q = O($);
        if (q !== "object")
          return new w("Invalid " + Z + " `" + W + "` of type " + ("`" + q + "` supplied to `" + L + "`, expected an object."));
        for (var K in $)
          if (n($, K)) {
            var X = E($, K, L, Z, W + "." + K, r);
            if (X instanceof Error)
    ...
    function U(E) {
      function f(R, H, L, Z, W) {
        var $ = R[H], q = O($);
        if (q !== "object")
          return new w("Invalid " + Z + " `" + W + "` of type `" + q + "` " + ("supplied to `" + L + "`, expected `object`."));
        for (var K in E) {
          var X = E[K];
  ...
  const [e, t] = se(hn), r = we(), n = k((c, l, x = null) => {
    ...
    try {
      ...
      if (!C)
        return !1;
      if (!b) {
        const D = x.startContainer, N = D.nodeType === Node.TEXT_NODE ? D.parentElement : D, m = Qe(N) || Br();
        return m && (m.style.textAlign = C, s(C)), !0;
      }
      const j = jr(x);
      if (j.length === 0) {
    ...
    try {
      ...
      if (x) {
        const N = l.startContainer, m = N.nodeType === Node.TEXT_NODE ? N.parentElement : N;
        if (m) {
          const g = window.getComputedStyle(m), y = g.fontSize, M = g.fontFamily;
          y && y.endsWith("px") && (b = y), M && M !== "serif" && (w = M.split(",")[0].replace(/['"]/g, "").trim());
          const S = Qe(m);
          if (S) {
...
const Te = 'page-break, [data-page-break="true"]', Nn = (e) => _e[e] || _e.A4, zr = (e, t = {}) => {
    ...
    for (let l = 0; l < u.length && c < yn; l++) {
      ...
      if (b && b.overflowIndex > 0) {
        ...
        for (let N = 0; N < x.length; N++) {
          const m = x[N];
          m && m.getBoundingClientRect && (w += Ln(m));
        }
        if (w - p < Kr)
          continue;
        const j = l + 2;
  ...
  const c = k((y = {}) => t?.current ? zr(t.current, y) : [], [t]), l = k((y = {}) => {
    const M = c(y);
    return a.updatePageBoundaries(M), M;
  }, [c, a]), x = k((y = {}) => {
    const M = typeof y.delay == "number" ? y.delay : oo;
    no(l, o, y, M);
  }, [l]), b = k(() => {
    t?.current && Jr(
      t.current,
    ...
    Qr(b, s, y);
  }, [b]), C = k((y) => !y?.current || !t?.current ? 0 : $r(y.current, t.current), [t]), j = k((y) => {
    ...
    to(M.current, t.current, y, j);
  }, [t, j]), N = k(async (y) => t?.current ? Wr(
    t.current,
    y,
    i.current,
    p.current,
    ...
    j,
    b
  ) : !1, [t, c, a, l, j, b]), g = k((y) => t?.current ? Zr(
    y,
    t.current,
    a.updateContinuousContent,
    l,
...
], Js = '<table border="1"><tr><th>Header 1</th><th>Header 2</th></tr><tr><td>Cell 3</td><td>Cell 4</td></tr></table>', Lt = ({
  currentFormat: e,
  onFormatText: t,
  onAddPageBreak: r,
  canUndo: n = !1,
...
const xe = (e) => e ? e.nodeType === Node.ELEMENT_NODE ? e.tagName === "PAGE-BREAK" || e.getAttribute("data-page-break") === "true" : e.parentElement && xe(e.parentElement) : !1, It = ({
  ...
  const c = ve(() => xn(d), [d]), l = u / 100, x = k((m, g) => {
    if (!m) return !1;
    const y = m.getBoundingClientRect(), M = g - y.top;
    return M < c.top || M > y.height - c.bottom;
  }, [c.top, c.bottom]), b = k((m, g) => {
    g.preventDefault();
    const y = m.getBoundingClientRect(), M = g.clientX - y.left, S = g.clientY - y.top, z = y.width - 40, G = -12, P = y.width - 20;
    M >= z && M <= P && S >= G && S <= 8 && p && p(m);
  }, [p]), w = k((m) => {
    m.preventDefault();
    ...
    const y = g.getRangeAt(0), { startContainer: M, endContainer: S } = y;
    if ((xe(M) || xe(S)) && (m.key.length === 1 || m.key === "Enter" || m.key === " "))
      return m.preventDefault(), !1;
    if (Vs(m))
      return n && n(), !1;
    if ((m.key === "Backspace" || m.key === "Delete") && (xe(M) || xe(S)))
      return m.preventDefault(), !1;
    a && a(m);
  }, [a, n]), j = k((m) => {
    if (!t.current) return;
    ...
    const g = window.getSelection();
    if (!g || g.rangeCount === 0) return;
    const y = g.getRangeAt(0), { startContainer: M } = y;
    if (xe(M))
      return m.preventDefault(), !1;
  }, []), N = k((m) => {
    const g = ea(m);
...
const ra = pr(It), Dt = ({
  ...
  return /* @__PURE__ */ h.jsxs("div", { className: "page-manager", children: [
    ...
    /* @__PURE__ */ h.jsx("div", { className: "page-list", children: e.map((g, y) => /* @__PURE__ */ h.jsxs("div", { className: "page-item", children: [
      /* @__PURE__ */ h.jsxs(
        "button",
        {
          type: "button",
...
const Ht = ({
  ...
  const l = k((m) => {
    if (!be(m)) return;
    o.current && (pt(o.current), o.current = null), m.classList.add("selected");
    const g = ao(m, u.current);
    g && (o.current = g, s.current = m, document.body.appendChild(g), g.querySelectorAll(".resize-handler").forEach((M) => {
    ...
    if (m.preventDefault(), m.stopPropagation(), !s.current) return;
    const g = m.currentTarget.dataset.handler, y = s.current.getBoundingClientRect(), M = window.pageYOffset || document.documentElement.scrollTop, S = window.pageXOffset || document.documentElement.scrollLeft;
    i.current = {
      x: m.clientX,
      y: m.clientY,
      ...
      offsetX: m.clientX - y.left - S,
      offsetY: m.clientY - y.top - M
    ...
    const { x: g, y, width: M, height: S } = i.current, z = p.current, G = m.clientX, P = m.clientY, U = aa({
      handler: z,
      startX: g,
      startY: y,
      currentX: G,
    ...
    if (d.current) {
      if (m.preventDefault(), m.stopPropagation(), s.current && i.current) {
        const g = {
        ...
        (g.width !== y.width || g.height !== y.height) && c.recordOperation(
    ...
    o.current && !o.current.contains(m.target) && x();
    const g = m.target;
    if (be(g)) {
      if (g === s.current)
        return;
    ...
    if ((m.key === "Delete" || m.key === "Backspace") && s.current && !d.current) {
      m.preventDefault();
      const g = s.current;
      if (g.parentNode) {
    ...
    m.key === "Escape" && s.current && x(), s.current && !d.current && ca(m, e, s.current, u.current);
  }, [e, x]), N = k(() => {
  ...
  return ae(() => {
    ...
    if (!e.current) return;
    const m = e.current;
    return m.addEventListener("click", j), m.addEventListener("keydown", D), window.addEventListener("resize", N), () => {
      m.removeEventListener("click", j), m.removeEventListener("keydown", D), window.removeEventListener("resize", N), d.current && (document.removeEventListener("mousemove", w), document.removeEventListener("mouseup", C), document.body.classList.remove("resize-in-progress"), document.body.style.cursor = ""), o.current && pt(o.current);
    };
  }, [e, j, D, N, w, C]), ae(() => {
    if (!e.current) return;
...
const Bt = ({
  ...
  const w = k(() => {
    ...
    const z = S.currentTarget, G = z.dataset.type, P = parseInt(z.dataset.index, 10);
    ...
    i.current = {
      ...
      y: S.clientY,
      structure: { ...U }
    }, p.current = G, u.current = P, c.current = !0, document.body.classList.add("table-resize-in-progress");
    const v = {
    ...
    document.body.style.cursor = v[G] || "default", document.addEventListener("mousemove", N), document.addEventListener("mouseup", m);
  }, []), j = k((S) => {
    ...
    const { x: z, y: G, structure: P } = i.current, U = p.current, v = u.current, B = S.clientX, A = S.clientY;
    let O = 0;
    switch (U) {
      case le.COLUMN:
        O = B - z;
    ...
    if (c.current) {
      if (S.preventDefault(), S.stopPropagation(), s.current && i.current) {
        ...
        (z.structure.width !== G.structure.width || z.structure.height !== G.structure.height || JSON.stringify(z.structure.columnWidths) !== JSON.stringify(G.structure.columnWidths) || JSON.stringify(z.structure.rowHeights) !== JSON.stringify(G.structure.rowHeights)) && b.recordOperation(
          { type: "TABLE_RESIZE", payload: { element: s.current, state: G } },
          { type: "TABLE_RESIZE", payload: { element: s.current, state: z } }
        );
      }
      if (i.current = null, p.current = null, u.current = null, c.current = !1, document.removeEventListener("mousemove", N), document.removeEventListener("mouseup", m), document.body.classList.remove("table-resize-in-progress"), document.body.style.cursor = "", s.current) {
    ...
    const G = S.target.closest("table");
    if (G && ue(G)) {
      if (G === s.current)
        return;
      D(G);
  ...
  return ae(() => {
    ...
    if (!e.current) return;
    const S = e.current;
    return S.addEventListener("click", g), S.addEventListener("keydown", y), window.addEventListener("resize", M), () => {
      S.removeEventListener("click", g), S.removeEventListener("keydown", y), window.removeEventListener("resize", M), c.current && (document.removeEventListener("mousemove", N), document.removeEventListener("mouseup", m), document.body.classList.remove("table-resize-in-progress"), document.body.style.cursor = ""), o.current && Ve(o.current);
    };
  }, [e, g, y, M, N, m]), ae(() => {
    if (!e.current) return;
    const S = new MutationObserver((z) => {
      z.forEach((G) => {
        G.type === "childList" && (G.removedNodes.forEach((P) => {
          P.nodeType === Node.ELEMENT_NODE && ue(P) && P === l && w();
        }), G.addedNodes.forEach((P) => {
...
const $t = ({
  ...
  return ae(() => {
    ...
    return P.addEventListener("mousedown", U, !0), P.addEventListener("mouseenter", v, !0), document.addEventListener("mouseup", z), document.addEventListener("click", G), () => {
      P.removeEventListener("mousedown", U, !0), P.removeEventListener("mouseenter", v, !0), document.removeEventListener("mouseup", z), document.removeEventListener("click", G);
    };
  }, [e, M, S, z, G]), ae(() => {
    if (!e.current) return;
...
const ya = 50, gt = 50, ba = 50, va = 100, Ta = 300, Ut = St(({
  ...
  const c = _n(), l = we(), { pageSize: x, pageMargins: b, continuousContent: w, pageBoundaries: C, activePage: j, zoomLevel: D, canUndo: N, canRedo: m } = c, g = ee(null), y = ee(null), { currentFormat: M, formatText: S, updateCurrentFormatFromSelection: z } = Hr(), [G, P] = se(!1), [U, v] = se(null), [B, A] = se(!0), [O, V] = se(!1), [oe, te] = se(null), [E, f] = se(null), [R, H] = se(null), L = ee(null), {
  ...
  } = so(x, y, D, b), de = ve(() => Ot(x), [x]), [Re, He] = se(!1), Ee = ee(null), je = ee(!1), Be = ee(!1), ot = ee(!1), Se = ee(w);
  ae(() => {
    if (y.current) {
      if (!ot.current) {
        y.current.innerHTML = w, ot.current = !0, Se.current = w;
        const _ = setTimeout(() => {
          q();
        }, ya);
      ...
      if (Se.current !== w) {
        if (y.current.innerHTML !== w) {
          y.current.innerHTML = w;
          const Q = setTimeout(() => {
            q(), je.current && Wt();
          }, gt);
  ...
  const Ft = ve(() => ({
     ...
     */
    getHTMLContent: () => y.current ? y.current.innerHTML : w,
    /**
     * Get the plain text content (HTML stripped)
     * @returns {string} Plain text content
     */
    getPlainText: () => (y.current ? y.current.innerHTML : w).replace(/<[^>]*>/g, " ").replace(/&nbsp;/g, " ").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/\s+/g, " ").trim(),
    /**
     * Get the current cursor position in the editor
     * @returns {Object|null} An object with page number, line number, and character offset, or null if no selection
     */
    getCursorPosition: () => y.current ? ro(y.current) : null,
    /**
    ...
    setContent: (_) => {
      const Q = Xe(_);
      Q !== w && l.updateContinuousContent(Q), y.current && y.current.innerHTML !== Q && (y.current.innerHTML = Q), Se.current = Q, y.current && setTimeout(() => {
        q();
      }, gt);
    },
    /**
  ...
  gr(d, () => Ft, [Ft]), ae(() => {
    const _ = () => {
      if (z(), y.current) {
        ...
        if (ie.rangeCount > 0) {
          ...
          fe.collapsed && y.current.contains(fe.commonAncestorContainer) && (L.current = fe.cloneRange());
        }
      }
    };
    document.addEventListener("selectionchange", _);
    const Q = () => {
      if (setTimeout(z, 10), L.current && y.current)
        try {
          const ie = window.getSelection();
          ie.removeAllRanges(), ie.addRange(L.current);
        } catch (ie) {
    ...
    return y.current && y.current.addEventListener("focus", Q), () => {
      document.removeEventListener("selectionchange", _), y.current && y.current.removeEventListener("focus", Q);
    };
  }, [z]);
  const Wt = k(() => {
    je.current = !1;
  ...
  }, [l, Z, W, j, K, g, y, w]), er = k((_) => {
    st();
  }, [st]), qt = k((_) => {
    l.updatePageSize(_), q({ pageSize: _ }), a && a(_);
  }, [l, q, a]), Gt = k((_) => {
    l.updatePageMargins(_), q({ pageMargins: _ }), o && o(_);
  }, [l, q, o]), Zt = k((_) => {
  ...
  ae(() => {
    const _ = (Q) => {
      if (Q.ctrlKey || Q.metaKey) {
        ...
        const ie = document.activeElement === y.current, fe = document.activeElement === g.current, ye = document.activeElement?.tagName === "INPUT" || document.activeElement?.tagName === "TEXTAREA" || document.activeElement?.isContentEditable === !1;
        if (ie || fe || !ye)
          switch (Q.key) {
            case "+":
            case "=":
  ...
  const rr = k(() => {
    !g.current || !y.current || Be.current || (Ee.current && clearTimeout(Ee.current), Ee.current = setTimeout(() => {
      if (Be.current) {
        Ee.current = null;
        return;
      }
