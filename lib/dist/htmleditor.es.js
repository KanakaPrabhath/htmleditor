import Xe, { createContext as ar, useContext as lr, useReducer as cr, useMemo as J, useState as K, useCallback as L, useRef as G, useEffect as q, forwardRef as vt, createElement as ut, memo as ur, useLayoutEffect as dn, useImperativeHandle as dr } from "react";
...
React keys must be passed directly to JSX without using spread:
  ...
  <%s key={someKey} {...props} />`,
    ...
    var p = Xe, f = Symbol.for("react.transitional.element"), T = Symbol.for("react.portal"), x = Symbol.for("react.fragment"), A = Symbol.for("react.strict_mode"), R = Symbol.for("react.profiler"), N = Symbol.for("react.consumer"), w = Symbol.for("react.context"), _ = Symbol.for("react.forward_ref"), m = Symbol.for("react.suspense"), k = Symbol.for("react.suspense_list"), v = Symbol.for("react.memo"), O = Symbol.for("react.lazy"), M = Symbol.for("react.activity"), b = Symbol.for("react.client.reference"), P = p.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE, S = Object.prototype.hasOwnProperty, j = Array.isArray, F = console.createTask ? console.createTask : function() {
      return null;
    };
    p = {
...
var Zt;
function fr() {
  return Zt || (Zt = 1, process.env.NODE_ENV === "production" ? ze.exports = gr() : ze.exports = pr()), ze.exports;
}
var l = fr();
const dt = 100, gt = 50, pt = 200, Vt = 5, ft = (e) => e < pt, ht = (e) => e > gt, hr = 96, ce = (e) => Math.round(e * hr), Ze = {
  NORMAL: {
    name: "Normal",
    label: 'Normal (1")',
    top: 1,
...
`) || fe : e.continuousContent && e.continuousContent !== fe && (o = [{ ...Le(0, e.pageSize), content: e.continuousContent }]), {
  ...
  const [e, t] = K(Yt), n = xe(), r = L((p, f, T = null) => {
    try {
      ...
      try {
        ...
        return N.appendChild(w), A.insertNode(N), A.selectNodeContents(N), x.removeAllRanges(), x.addRange(A), t((_) => ({ ..._, [p]: f })), !0;
      } catch {
        if (T) {
          const N = p === "fontSize" ? Ar[f] || "3" : f;
          return document.execCommand(T, !1, N), t((w) => ({ ...w, [p]: f })), !0;
        }
        return !1;
      }
    } catch (x) {
    ...
    try {
      ...
      if (!R)
        return !1;
      if (!x) {
        const w = T.startContainer, _ = w.nodeType === Node.TEXT_NODE ? w.parentElement : w, m = Ne(_) || Lr();
    ...
    try {
      ...
      if (x.innerHTML = p, Array.from(x.children).some(
        ...
        const R = T.startContainer, N = R.nodeType === Node.TEXT_NODE ? R.parentElement : R, w = Ne(N);
        if (w && ["P", "H1", "H2", "H3", "H4", "H5", "H6"].includes(w.tagName)) {
          ...
          if (T.startOffset === 0 && (R === w || R.nodeType === Node.TEXT_NODE && R === w.firstChild))
            ...
            const v = T.extractContents(), O = document.createTextNode(""), M = document.createElement(_);
            M.appendChild(O), M.appendChild(v);
            const b = w.parentNode;
            b.insertBefore(x, w.nextSibling), b.insertBefore(M, x.nextSibling), !w.textContent.trim() && w.children.length === 0 && b.removeChild(w);
          }
        } else
          T.insertNode(x);
    ...
    try {
      ...
      let x = Je, A = "Arial", R = "#000000", N = "p", w = "left";
      if (T) {
        const m = f.startContainer, k = m.nodeType === Node.TEXT_NODE ? m.parentElement : m;
        if (k) {
          const v = window.getComputedStyle(k), O = v.fontSize, M = v.fontFamily;
          O && O.endsWith("px") && (x = O), M && M !== "serif" && (A = M.split(",")[0].replace(/['"]/g, "").trim());
          const b = v.color;
          b && b !== "rgb(0, 0, 0)" && b !== "#000000" && (b.startsWith("rgb") ? R = Rr(b) : R = b);
          const P = Ne(k);
          if (P) {
            const S = P.tagName?.toLowerCase();
            ["h1", "h2", "h3", "h4", "h5", "h6", "p"].includes(S) && (N = S);
...
const ge = 'page-break, [data-page-break="true"]', Nr = (e, t = {}) => {
  ...
  try {
    ...
    const c = En(t, n, r);
    let u = Kt(e), h = !1, d = 0;
    for (let g = 0; g < u.length && d < Xt; g++) {
      const p = u[g], f = zr(p, c);
      if (f && f.overflowIndex > 0) {
        let T = 0;
        for (let N = 0; N < p.length; N++) {
          const w = p[N];
          w && w.getBoundingClientRect && (T += bn(w));
        }
        if (T - c < Dr)
          continue;
        const A = g + 2;
        _r(f.overflowElement, A) && (h = !0, d++, u = Kt(e));
      }
    }
    if (h) {
      if (i) {
  ...
  const d = L((m = {}) => t?.current ? Nr(t.current, m) : [], [t]), g = L((m = {}) => {
    const k = d(m);
    ...
    Fr(f, s, m);
  }, [f]), x = L((m) => !m?.current || !t?.current ? 0 : Or(m.current, t.current), [t]), A = L((m) => {
...
const us = [
  ...
  const i = yn(), { pages: o, activePage: s, continuousContent: a } = i, { wordCount: c, outline: u } = J(() => {
    if (n !== void 0 && r !== void 0)
      return {
        wordCount: n,
        outline: Qt(a, 1, "continuous-")
      };
    let d = 0;
    const g = [];
    return o.forEach((p, f) => {
      d += gs(p.content), g.push(...Qt(p.content, f + 1, `page-${f}-`));
    ...
    s.onload = (a) => {
      ...
      try {
        ...
        const T = e.type === "image/png" && Ps(f, a, c) ? "image/png" : "image/jpeg", x = await On(
          ...
          n.minQuality,
          n.qualityStep
        ), A = T === "image/png" ? ".png" : ".jpg", R = e.name.replace(/\.[^/.]+$/, "") + A, N = new File([x], R, { type: T }), w = N.size < e.size;
        r({
          file: N,
          width: a,
          height: c,
          originalSize: e.size,
          optimizedSize: N.size,
          wasOptimized: g || w,
          wasResized: g,
          wasCompressed: w,
          originalWidth: u,
...
async function On(e, t, n, r, i, o) {
  let s = r, a = null;
  if (t === "image/png")
    return a = await en(e, t, s), a.size > n ? On(e, "image/jpeg", n, r, i, o) : a;
  for (; s >= i; ) {
    if (a = await en(e, t, s), a.size <= n)
      return a;
    s -= o;
  }
  return a;
...
function Is(e) {
  ...
  const t = [];
  return e.wasResized && t.push(`resized from ${e.originalWidth}×${e.originalHeight} to ${e.width}×${e.height}`), e.wasCompressed && t.push(`compressed from ${tn(e.originalSize)} to ${tn(e.optimizedSize)}`), `Image optimized: ${t.join(", ")}`;
}
class js {
...
const Fs = [
...
], Ws = '<table border="1"><tr><th>Header 1</th><th>Header 2</th></tr><tr><td>Cell 3</td><td>Cell 4</td></tr></table>', Ct = ({
  currentFormat: e,
  onFormatText: t,
  onAddPageBreak: n,
  onInsertImage: r,
  ...
  const s = (p, f, T, x = null, A = !1) => /* @__PURE__ */ l.jsx(
    ...
    try {
      ...
      const R = await no(A), N = await ro(R);
      if (N && r) {
        const w = `<img src="${N}" data-key="${R}" alt="Inserted image" style="max-width: 100%;" />`;
        r(w);
      } else
...
const Ys = 20, Ks = 20, nn = 20, rn = -12, Xs = {
  position: "relative",
  width: "100%",
  maxWidth: "100%",
  margin: "40px auto",
...
}, Qs = (e, t, n) => !!((me(t) || me(n)) && (e.key.length === 1 || e.key === "Enter" || e.key === " ")), ei = (e, t, n) => !!((e.key === "Backspace" || e.key === "Delete") && (me(t) || me(n))), me = (e) => e ? e.nodeType === Node.ELEMENT_NODE ? e.tagName === "PAGE-BREAK" || e.getAttribute("data-page-break") === "true" : e.parentElement && me(e.parentElement) : !1, Rt = ({
  dimensions: e,
  editorRef: t,
  onInput: n,
  onContentChange: r,
...
const kt = ({
  ...
  const c = L((x) => {
    ...
    A && (o.current.overlay = A, o.current.image = x, document.body.appendChild(A), A.querySelectorAll(".resize-handler").forEach((N) => {
    ...
    const A = x.currentTarget.dataset.handler, R = o.current.image.getBoundingClientRect(), N = window.pageYOffset || document.documentElement.scrollTop, w = window.pageXOffset || document.documentElement.scrollLeft;
    o.current.start = {
      x: x.clientX,
      y: x.clientY,
    ...
    const { x: A, y: R, width: N, height: w } = o.current.start, _ = o.current.handler, m = x.clientX, k = x.clientY, v = ai({
      handler: _,
      startX: A,
      startY: R,
      currentX: m,
    ...
    jn(o.current.image, v), o.current.overlay && yt(o.current.overlay, o.current.image);
  }, []), g = L((x) => {
  ...
  return q(() => {
    ...
    const x = new MutationObserver((A) => {
      A.forEach((R) => {
        R.type === "childList" && R.removedNodes.forEach((N) => {
          N.nodeType === Node.ELEMENT_NODE && he(N) && N === o.current.image && u();
        });
      });
    });
...
const Lt = {
  ...
  const [s, a] = K({ top: 0, left: 0 }), [c, u] = K(!1), [h, d] = K("top"), [g, p] = K(o), f = G(null), T = xe(), x = (b) => {
  ...
  const _ = L(() => {
    ...
    const b = e.getBoundingClientRect(), P = f.current.getBoundingClientRect(), S = window.innerWidth, j = window.innerHeight, F = b.bottom >= 0 && b.top <= j && b.right >= 0 && b.left <= S;
    let z = "top", V = 0, X = 0;
    if (F) {
      const E = b.top - P.height - 10, H = j - b.bottom - P.height - 10;
      H > E && H > 0 ? (z = "bottom", V = b.bottom + 10) : (z = "top", V = b.top - P.height - 10);
    } else
      z = "top", V = 10;
    X = b.left + b.width / 2 - P.width / 2;
    let re = Math.max(10, Math.min(X, S - P.width - 10)), le = Math.max(10, Math.min(V, j - P.height - 10));
  ...
  const m = {
    ...
    if (e && m[b]) {
      const P = A(e), S = N, j = m[b];
      e.style.float = j.float, e.style.margin = j.margin, e.style.display = j.display, w(b), x(e);
      const F = A(e);
      T.recordOperation(
        { type: "IMAGE_ALIGN", payload: { element: e, alignment: b, state: F } },
        { type: "IMAGE_ALIGN", payload: { element: e, alignment: S, state: P } }
      ), t && t(b);
    }
  }, v = () => {
    if (e && e.parentNode) {
    ...
    try {
      if (b.startsWith("blob:"))
        ...
        const F = new URL(b).pathname.split("/"), z = F[F.length - 1];
        z && z.includes(".") && (P = z);
      }
    } catch {
      P = "image.png";
    ...
    S.href = b, S.download = P, S.style.display = "none", document.body.appendChild(S), S.click(), document.body.removeChild(S);
  };
...
function zn(e, t, n) {
  return St(e, `delete${n.charAt(0).toUpperCase() + n.slice(1)}`) ? n === "row" ? Ri(e, t) : n === "column" ? ki(e, t) : !1 : !1;
}
function Ri(e, t) {
...
const _t = ({
  ...
  const T = L(() => {
    ...
    const O = v.currentTarget, M = O.dataset.type, b = parseInt(O.dataset.index, 10);
    ...
    a.current = {
      ...
      y: v.clientY,
      structure: { ...P }
    }, c.current = M, u.current = b, d.current = !0, document.body.classList.add("table-resize-in-progress");
    const j = {
      [$.COLUMN]: "col-resize",
      [$.ROW]: "row-resize",
    ...
    document.body.style.cursor = j, document.addEventListener("mousemove", N), document.addEventListener("mouseup", w);
  }, []), A = L((v) => {
    const O = fi(v, h.current);
    return O && (o.current = O, s.current = v, document.body.appendChild(O), O.querySelectorAll(".table-resize-handler").forEach((b) => {
    ...
    qe(v);
    const { x: O, y: M, structure: b } = a.current, P = c.current, S = u.current, j = v.clientX, F = v.clientY;
    let z = 0;
    switch (P) {
      case $.COLUMN:
        z = j - O;
    ...
    Pe(s.current, V), o.current && wt(o.current, s.current);
  }, []), w = L((v) => {
    if (d.current) {
      if (qe(v), s.current && a.current) {
        ...
        (O.structure.width !== M.structure.width || O.structure.height !== M.structure.height || JSON.stringify(O.structure.columnWidths) !== JSON.stringify(M.structure.columnWidths) || JSON.stringify(O.structure.rowHeights) !== JSON.stringify(M.structure.rowHeights)) && f.recordOperation(
          { type: "TABLE_RESIZE", payload: { element: s.current, state: M } },
          { type: "TABLE_RESIZE", payload: { element: s.current, state: O } }
        );
      }
      if (a.current = null, c.current = null, u.current = null, d.current = !1, document.removeEventListener("mousemove", N), document.removeEventListener("mouseup", w), document.body.classList.remove("table-resize-in-progress"), document.body.style.cursor = "", s.current) {
        const O = new Event("input", { bubbles: !0, cancelable: !0 });
        s.current.dispatchEvent(O), t && t(s.current, ae(s.current));
      }
    }
  }, [t, f]), _ = L((v) => {
    o.current && !o.current.contains(v.target) && T();
    const M = v.target.closest("table");
    if (M && ie(M)) {
      if (M === s.current)
        return;
      R(M);
  ...
  return q(() => {
    ...
    return O && (O.addEventListener("scroll", k), window.addEventListener("scroll", k)), () => {
      O && O.removeEventListener("scroll", k), window.removeEventListener("scroll", k);
    };
  }, [e, k]), q(() => {
    if (!e.current) return;
    const v = e.current;
    return v.addEventListener("click", _), v.addEventListener("keydown", m), window.addEventListener("resize", k), () => {
      v.removeEventListener("click", _), v.removeEventListener("keydown", m), window.removeEventListener("resize", k), d.current && (document.removeEventListener("mousemove", N), document.removeEventListener("mouseup", w), document.body.classList.remove("table-resize-in-progress"), document.body.style.cursor = ""), o.current && ke(o.current);
    };
  }, [e, _, m, k, N, w]), q(() => {
    if (!e.current) return;
    const v = new MutationObserver((O) => {
      O.forEach((M) => {
        M.type === "childList" ? (M.removedNodes.forEach((b) => {
          b.nodeType === Node.ELEMENT_NODE && ie(b) && b === g && T();
        }), M.addedNodes.forEach((b) => {
          if (b.nodeType !== Node.ELEMENT_NODE) return;
          if (ie(b)) {
            setTimeout(() => R(b), 50);
            return;
          }
          const P = b.querySelectorAll?.("table") || [];
          P.length > 0 && setTimeout(() => R(P[0]), 50);
        }), g && [...M.addedNodes, ...M.removedNodes].some(
          (P) => P.nodeType === Node.ELEMENT_NODE && P.tagName === "TR"
        ) && o.current && s.current && (ke(o.current), o.current = null, A(s.current))) : M.type === "attributes" && ["colspan", "rowspan"].includes(M.attributeName) && g && o.current && s.current && (ke(o.current), o.current = null, A(s.current));
      });
    });
...
const Ue = {
  ...
  const [s, a] = K({ top: 0, left: 0 }), [c, u] = K(!1), [h, d] = K("top"), [g, p] = K("left"), f = G(null), T = xe(), x = L(() => {
    if (!e) return "left";
  ...
  const A = L(() => {
    if (!e || !f.current) return;
    const S = e.getBoundingClientRect(), j = f.current.getBoundingClientRect(), F = window.visualViewport ? window.visualViewport : {
      width: window.innerWidth,
      height: window.innerHeight
    }, z = S.bottom >= 0 && S.top <= F.height && S.right >= 0 && S.left <= F.width;
    let V = "top", X = 0, re = 0;
    if (z) {
      const Y = S.top - j.height - 10, Q = F.height - S.bottom - j.height - 10;
      Q > Y && Q > 0 ? (V = "bottom", X = S.bottom + 10) : (V = "top", X = S.top - j.height - 10);
    } else
      V = "top", X = 10;
    re = S.left + S.width / 2 - j.width / 2;
  ...
  dn(() => {
    ...
    return window.addEventListener("scroll", S), window.addEventListener("resize", S), () => {
      window.removeEventListener("scroll", S), window.removeEventListener("resize", S), u(!1);
    };
  }, [e, A]), q(() => {
    const S = (j) => {
  ...
  const R = L((S) => {
    if (!e || !an[S]) return;
    const j = cn(e), F = g, z = an[S];
    e.style.float = z.float, e.style.margin = z.margin, e.style.marginLeft = z.marginLeft, e.style.marginRight = z.marginRight, p(S), ln(e);
    const V = cn(e);
    T.recordOperation(
      { type: "TABLE_ALIGN", payload: { element: e, alignment: S, state: V } },
    ...
    () => N(be.INSERT_ROW_ABOVE, xi),
    [N]
  ), _ = J(
    () => N(be.INSERT_ROW_BELOW, yi),
    [N]
  ), m = J(
    () => N(be.INSERT_COL_LEFT, Ei),
    [N]
  ), k = J(
    () => N(be.INSERT_COL_RIGHT, bi),
    [N]
  ), v = J(
    () => N(be.DELETE_ROW, Li),
    [N]
      ...
      "div",
      {
        ...
        children: [
          ...
          r != null && /* @__PURE__ */ l.jsxs(l.Fragment, { children: [
            /* @__PURE__ */ l.jsx("div", { style: b.divider }),
            /* @__PURE__ */ l.jsx(
              "button",
              {
...
const Oi = ".table-cell-selected, .table-row-selected, .table-col-selected", un = () => {
  ...
  const n = G({
    ...
    hasDragged: !1,
    startCell: null
  }), [r, i] = K(!1), [o, s] = K(!1), [a, c] = K(null), [u, h] = K(null), d = G(null), g = L(() => {
    n.current = {
      selectedCells: /* @__PURE__ */ new Set(),
      selectionMode: null,
      selectedRowIndex: null,
    ...
    if (!k || !v) return null;
    const O = Array.from(v.rows).indexOf(k), M = Array.from(k.cells).indexOf(m);
    return { rowIndex: O, cellIndex: M, table: v };
  }, []), T = L((m) => {
    if (m.size === 0) return { mode: null, rowIndex: null, colIndex: null };
    const k = Array.from(m), v = k[0], O = f(v);
    if (!O) return { mode: null, rowIndex: null, colIndex: null };
    const { table: M } = O;
    return k.filter((S) => {
      const j = f(S);
      return j && j.table === M && j.rowIndex === O.rowIndex;
    }).length >= 2 ? { mode: "row", rowIndex: O.rowIndex, colIndex: null } : k.filter((S) => {
      const j = f(S);
      return j && j.table === M && j.cellIndex === O.cellIndex;
    }).length >= 2 ? { mode: "column", rowIndex: null, colIndex: O.cellIndex } : { mode: null, rowIndex: null, colIndex: null };
  }, [f]), x = L((m, k, v, O, M) => {
    if (un(), !(!M || !m)) {
      if (m === "row" && k !== null) {
        const b = M.rows[k];
        b && Array.from(b.cells).forEach((P) => {
          P.classList.add("table-row-selected");
        });
      } else m === "column" && v !== null ? Array.from(M.rows).forEach((b) => {
        const P = b.cells[v];
        P && P.classList.add("table-col-selected");
      }) : m === "cells" && O.forEach((b) => {
        b.classList.add("table-cell-selected");
      });
      gi();
    }
  }, []), A = L((m, k) => {
    ...
    JSON.stringify(d.current) !== JSON.stringify(O) && (d.current = O, t && t(O));
  }, [T, x, t]), R = L((m) => {
    ...
    if (!r || !a) return;
    const k = m.target.closest("td, th");
    if (!k || !u || !sn(u, k)) return;
    s(!0), qe(m);
    const v = f(a), O = f(k);
    if (!v || !O || v.table !== O.table) return;
    const M = Math.min(v.rowIndex, O.rowIndex), b = Math.max(v.rowIndex, O.rowIndex), P = Math.min(v.cellIndex, O.cellIndex), S = Math.max(v.cellIndex, O.cellIndex), j = /* @__PURE__ */ new Set();
    for (let F = M; F <= b; F++) {
      const z = u.rows[F];
      if (z)
        for (let V = P; V <= S; V++) {
  ...
  return q(() => {
    if (!e.current) return;
    const m = e.current, k = (O) => {
      O.target.closest("td, th") && R(O);
    }, v = (O) => {
      O.target.closest("td, th") && N(O);
    };
...
const Si = 50, ct = 50, _i = 50, Mi = 100, Pi = 300, It = vt(({
  ...
  const d = yn(), g = xe(), { pageSize: p, pageMargins: f, continuousContent: T, pageBoundaries: x, activePage: A, zoomLevel: R, canUndo: N, canRedo: w } = d, _ = G(null), m = G(null), { currentFormat: k, formatText: v, updateCurrentFormatFromSelection: O } = Cr(), [M, b] = K(!1), [P, S] = K(null), [j, F] = K(!0), [z, V] = K(!1), [X, re] = K(null), [le, E] = K(null), [H, U] = K(null), {
  ...
  } = Vr(p, m, R, f), Fn = J(() => Se(p), [p]), [Un, Wn] = K(!1), pe = G(null), je = G(!1), Be = G(!1), He = G(!1), De = G(T), Qe = L(() => {
    je.current = !1;
  ...
  const { restoreCursorPosition: et, hasActiveCursorSelection: tt } = Kr({
    updateCurrentFormatFromSelection: O,
    editorRef: m
  }), Gn = L((C) => {
  ...
  const rt = L(() => {
    ...
    const C = x.length + 1;
    jt(C) && (je.current = !0, setTimeout(() => {
      je.current && Qe();
    }, 100)), n && n();
  }, [x.length, jt, g, se, n]), Zn = L(() => {
    const C = window.getSelection();
    if (C && C.rangeCount > 0) {
      if (!m?.current) return;
