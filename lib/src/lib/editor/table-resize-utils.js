 ...
 * @param {number} params.deltaX - Change in X direction (for table corner resize)
 * @param {number} params.deltaY - Change in Y direction (for table corner resize)
...
export function calculateTableResizeDimensions({
  ...
  switch (type) {
    case TABLE_RESIZE_HANDLERS.COLUMN:
      if (index >= 0 && index < result.columnWidths.length) {
        const newWidth = Math.max(
          options.minColumnWidth,
          Math.min(options.maxColumnWidth, result.columnWidths[index] + delta)
        );
        result.columnWidths[index] = newWidth;
        result.tableWidth = currentStructure.width;
      }
      break;

    case TABLE_RESIZE_HANDLERS.ROW:
      if (index >= 0 && index < result.rowHeights.length) {
        const newHeight = Math.max(
          options.minRowHeight,
          Math.min(options.maxRowHeight, result.rowHeights[index] + delta)
        );
        result.rowHeights[index] = newHeight;
        result.tableHeight = currentStructure.height;
      }
      break;

    case TABLE_RESIZE_HANDLERS.TABLE_BOTTOM:
      const lastRowIndex = currentStructure.rows - 1;
      if (lastRowIndex >= 0 && lastRowIndex < result.rowHeights.length) {
        const newHeight = Math.max(
          options.minRowHeight,
          Math.min(options.maxRowHeight, result.rowHeights[lastRowIndex] + delta)
        );
        result.rowHeights[lastRowIndex] = newHeight;
      }
      break;

    case TABLE_RESIZE_HANDLERS.TABLE_RIGHT:
      const lastColumnIndex = currentStructure.columns - 1;
      if (lastColumnIndex >= 0 && lastColumnIndex < result.columnWidths.length) {
        const newWidth = Math.max(
          options.minColumnWidth,
          Math.min(options.maxColumnWidth, result.columnWidths[lastColumnIndex] + delta)
        );
        result.columnWidths[lastColumnIndex] = newWidth;
      }
      break;
  }
...
export function applyTableDimensions(tableElement, dimensions, resizeType) {
  ...
  if (tableWidth !== undefined) {
    ...
    const sumWidth = columnWidths.reduce((sum, w) => sum + w, 0);
    tableElement.style.width = `${sumWidth}px`;
  }

  if (tableHeight !== undefined) {
    tableElement.style.height = `${tableHeight}px`;
  } else if (rowHeights && rowHeights.length > 0) {
    const sumHeight = rowHeights.reduce((sum, h) => sum + h, 0);
  ...
  if (columnWidths && columnWidths.length > 0) {
    ...
    // Set table width to sum of column widths to allow table to resize
    const sumWidth = columnWidths.reduce((sum, w) => sum + w, 0);
    tableElement.style.width = `${sumWidth}px`;
  }
  ...
  if (rowHeights && rowHeights.length > 0) {
    ...
    // Set table height to sum of row heights to allow table to resize
    const sumHeight = rowHeights.reduce((sum, h) => sum + h, 0);
    tableElement.style.height = `${sumHeight}px`;
  }
